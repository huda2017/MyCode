...
def train(model, criterion, criterion_st, optimizer, optimizer_st, scheduler,
    ...
    for num_iter, data in enumerate(data_loader):
        ...
        kl_weight = kl_anneal_function( c.VAE['anneal_function'], c.VAE['anneal_lag'], global_step, c.VAE['anneal_k'], c.VAE['anneal_x0'], c.VAE["anneal_upper"] )
    ...
    with torch.no_grad():
        if data_loader is not None:
            for num_iter, data in enumerate(data_loader):
                ...
                if c.bidirectional_decoder:
                    decoder_output, postnet_output, alignments, stop_tokens, decoder_backward_output, alignments_backward, mu, logvar, z = model(
                        text_input, text_lengths, mel_input, speaker_ids=speaker_ids,ref_cond=True)
                    _, postnet_output_noRef, _, _,_, _= model(text_input, text_lengths, mel_input, speaker_ids=speaker_ids,ref_cond=False)

                else:
                    decoder_output, postnet_output, alignments, stop_tokens, mu, logvar, z = model(
                        text_input, text_lengths, mel_input, speaker_ids=speaker_ids,ref_cond=True)
                    _, postnet_output_noRef, _, _ = model(text_input, text_lengths, mel_input, speaker_ids=speaker_ids,ref_cond=False)
            ...
            if args.rank == 0:
                ...
                if c.model in ["Tacotron", "TacotronGST"]:
                    ...
                    eval_audio_noRef = ap.inv_mel_spectrogram(const_spec_noRef.T)
                ...
                tb_logger.tb_eval_audios(global_step, {"ValAudioNoRef": eval_audio_noRef}, c.audio["sample_rate"])
                tb_logger.tb_eval_audios(global_step, {"RefAudio": tgruth_audio}, c.audio["sample_rate"])
